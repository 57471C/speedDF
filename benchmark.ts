...
async function runBenchmark() {
	const _data = new Uint8Array(100);
	const pdfDocument = {
		getOutline: async () => {
			const outline = [];
		...
		getPageIndex: async (destObj) => {
			await new Promise((r) => setTimeout(r, 1)); // simulate some async work
			return parseInt(destObj.split("_")[1], 10);
		},
	};
	...
	const _loadedBookmarksPar = await Promise.all(
