...
* **`src/lib/markdown/parse.ts`**: Markdown → HTML (`marked`, GFM). Pure projection — does not own document state. Fenced code uses `renderFencedBlock` (highlight.js).
* **`src/lib/markdown/highlight.ts`**: Shared **highlight.js** core + small language set (js/ts/rust/json/css/xml/python/bash/markdown). `highlightFenced` for preview fences; `highlightMarkdownSource` for the editor overlay (nested fence langs).
...
* **`src/lib/markdown/scrollSync.ts`**: Relative-position (`scrollTop / maxScroll`) editor ↔ preview sync. rAF-coalesced; applying flag blocks echo; typing guard skips preview → editor so the caret is not yanked.
...
* **`src/components/TitleBar.svelte`**: Custom OS-level header: window chrome, Extract Text (Lucide `square-dashed-text` icon), file open/save dialogs, orchestrates save via `lib/export/flatten` + `commitActiveDocumentAfterSave` (also re-extracts hyperlinks after save). Markdown: split-view toggle (columns icon) next to OCR; Save writes `encodeMarkdownSource(markdownSource)`.
* **`src/components/Workspace.svelte`**: Primary layout container mapping scrollable `WorkspacePage`s **or** continuous `MarkdownView` when `fileType === "markdown"` (optional `MarkdownSplitView` when `markdownSplitView`); **Ctrl/Cmd+wheel zoom-to-pointer** (content stack `data-workspace-pages`); hosts **text floating toolbar** + `ImageResizeStrip`; **dynamic `padding-top`** so page 1 clears floating strips; “Document loaded in Xms” burn-in. Print via `onPrint` (markdown uses iframe spool). Markdown zoom bar includes Split / Preview toggle.
* **`src/components/MarkdownView.svelte`**: Read-only continuous markdown projection. Prop: `source` (canonical UTF-8 string); optional `variant` (`document` card vs `pane` for split). Pipeline: parse → sanitize → **in-place `innerHTML`** on a stable host (no `{@html}` remount). After paint, schedules `captureMarkdownViewThumbnail` → `applyLiveThumbnail`. Typography + `@media print` styles. Live font is system-ui/Segoe UI sans (thumb capture re-asserts the same stack).
* **`src/components/MarkdownSourceEditor.svelte`**: Plain `<textarea>` bound to `markdownSource` via `setMarkdownSourceAction` (marks dirty). highlight.js overlay (transparent text, caret visible) for markdown + fenced-language colors. No WYSIWYG / vim / indent engine.
* **`src/components/MarkdownSplitView.svelte`**: Source left + live preview right (stacks source above preview when the host is narrow). Preview source is debounced; save always reads live `markdownSource`. Scroll panes stay aligned via `createMarkdownScrollSync` (ratio map, both directions). On preview HTML patch: `hold` → restore `scrollTop` → `release` so edits do not snap to top.
...
* **`markdownSplitView`**: Per-tab markdown source/preview split (default `false` = preview-only). Toggle via TitleBar, zoom-bar chip, or Ctrl+\\.
