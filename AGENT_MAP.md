...
* **`src/lib/markdown/parse.ts`**: Markdown → HTML (`marked`, GFM). Pure projection — does not own document state. Fenced code uses `renderFencedBlock` (highlight.js). Block tags are stamped with **`data-md-line`** (1-based source line) for split scroll.
* **`src/lib/markdown/highlight.ts`**: Shared **highlight.js/lib/core** + `registerLanguage` only (not the full package). Languages: js/ts, rust, json, css, xml, python, bash, markdown, plaintext, plus **yaml, sql, go, java, c, ini** (TOML is the ini grammar). Aliases include `yml`, `golang`, `h`, `jsp`, `toml`, and common SQL names. `highlightFenced` for preview fences; `highlightMarkdownSource` for the editor overlay. Fenced preview `<pre>` may carry **`data-lang`** for the corner label.
* **`src/lib/markdown/sanitize.ts`**: DOMPurify XSS sanitize for markdown HTML; keeps `class`, `data-lang`, and `data-md-line`; safe link `target`/`rel` hooks.
* **`src/lib/markdown/print.ts`**: Headless iframe print spool for markdown (`@page` A4, chrome-free).
* **`src/lib/markdown/source.ts`**: UTF-8 encode/decode for `.md` Save / reopen; preview debounce constant. Does not own document state.
* **`src/lib/markdown/scrollSync.ts`**: Editor ↔ preview sync by **source line** (`data-md-line` anchors), not `scrollTop / maxScroll`. Viewport-top line while scrolling; caret line while the typing guard is active. rAF-coalesced; applying flag blocks echo; typing guard skips preview → editor. Empty anchor map does nothing (no ratio fallback). `realign()` runs once after a preview image load. `hold` / `release` / 1px epsilon unchanged.
...
* **`src/components/TitleBar.svelte`**: Custom OS-level header: window chrome, Extract Text (Lucide `square-dashed-text` icon), file open/save dialogs, orchestrates save via `lib/export/flatten` + `commitActiveDocumentAfterSave` (also re-extracts hyperlinks after save). Markdown: labeled **Edit / Preview** button (not a new OS window) next to the other tools; Save writes `encodeMarkdownSource(markdownSource)`.
* **`src/components/Workspace.svelte`**: Primary layout container mapping scrollable `WorkspacePage`s **or** continuous `MarkdownView` when `fileType === "markdown"` (optional `MarkdownSplitView` when `markdownSplitView`); **Ctrl/Cmd+wheel zoom-to-pointer** (content stack `data-workspace-pages`); hosts **text floating toolbar** + `ImageResizeStrip`; **dynamic `padding-top`** so page 1 clears floating strips; “Document loaded in Xms” burn-in. Print via `onPrint` (markdown uses iframe spool). Markdown zoom bar includes the same **Edit / Preview** toggle.
* **`src/components/MarkdownView.svelte`**: Read-only continuous markdown projection. Prop: `source` (canonical UTF-8 string); optional `variant` (`document` card vs `pane` for split). Pipeline: parse → sanitize → **in-place `innerHTML`** on a stable host (no `{#key}` remount of the scroller). Page measure is about **78ch** on `--sdf-*` tokens (light and dark). Fenced preview uses `--sdf-hl-*` for keyword/string/comment/number/title and a `data-lang` corner label. After paint, schedules `captureMarkdownViewThumbnail` → `applyLiveThumbnail`. Typography + `@media print` styles. Live font is system-ui/Segoe UI sans (thumb capture re-asserts the same stack).
* **`src/components/MarkdownSourceEditor.svelte`**: Plain `<textarea>` bound to `markdownSource` via `setMarkdownSourceAction` (marks dirty). highlight.js overlay shares **13px / 20px** metrics, padding, font, `pre-wrap`, and tab size with the textarea (no `-webkit-text-fill-color: transparent`). Caret uses `--sdf-accent`. No WYSIWYG / vim / indent engine.
* **`src/components/MarkdownSplitView.svelte`**: Source left + live preview right. Draggable gutter sets **`markdownSplitRatio`** (clamped **0.25–0.75**, per document). Hosts narrower than 720px stack source above preview and **hide the gutter**. Preview source is debounced; save always reads live `markdownSource`. Scroll panes stay aligned via `createMarkdownScrollSync` (**line map**, both directions). On preview HTML patch: `hold` → restore `scrollTop` → `release` so edits do not snap to top.
...
* **`src/components/PageSidebar.svelte`**: Right-hand panel: page order, thumbnails, thumbnail options context menu (Add/Merge... & Insert Blank), outline/bookmarks, comments tab; **merge / blank-page insert** rebinds bytes + calls `refreshThumbnailsAfterPageInsert`. All cards & menus use design tokens. **Not mounted** when `fileType === "markdown"` (PDF and image unchanged). Recent documents still receive the live markdown thumb.
...
* **`markdownSplitView`**: Per-tab markdown source/preview split (default `false` = preview-only). Toggle via the title-bar **Edit / Preview** button, the zoom-bar chip, or Ctrl+\\. Does not open a new OS window.
* **`markdownSplitRatio`**: Per-tab source-pane fraction for the split row, clamped to **0.25–0.75** (default `0.5`). Ignored when the split stacks in a narrow column. Survives save; cleared back to `0.5` on purge.
...
* **`setMarkdownSourceAction(text)`**: Writes `markdownSource` on the active markdown tab and sets dirty. No-op while saving, if unchanged, or if the tab is not markdown.
* **`toggleMarkdownSplitView(force?)`**: Per-tab markdown source/preview split. No-op for PDF / image / TIFF.
...
**Last Updated:** September 2026 — **v1.2.5** (Markdown preview-first Edit/Preview, theme-token preview, wider page, fenced yaml/sql/go/java/c/ini, line-mapped split scroll, 25–75% gutter, page sidebar hidden for markdown; prior: SVG-as-image, continuous markdown viewer, Open in new window, image resize, multi-select, HEIC, forms, hyperlinks, workspaceId / Save As)
